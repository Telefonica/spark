    ...
    newlyCreatedExecutors.foreach { case (execId, timeCreated) =>
      ...
      if (currentTime - timeCreated > podCreationTimeout) {
        logWarning(s"Executor with id $execId was not detected in the Kubernetes" +
          ...
          s" deleted but the application missed the deletion event. TimeCreated: $timeCreated")
        ...
        logInfo(s"Executor with id $execId was not found in the Kubernetes cluster since it" +
          s" was created ${currentTime - timeCreated} milliseconds ago. TimeCreated: $timeCreated")
      }
    }
